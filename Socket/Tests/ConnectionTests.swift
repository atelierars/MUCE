...
struct ConnectionTests {
	...
	@Test
	func udpMessage() async throws {
		try await withThrowingDiscardingTaskGroup { group in
			group.addTask {
				for try await client in Connection.Incoming(on: 9841, with: .udp).values.prefix(1) {
					for try await packet in client.recv(count: 1024).values.prefix(1) {
						#expect(packet == req)
	...
	func tcpStream() async throws {
		try await withThrowingDiscardingTaskGroup { group in
			let stream = CurrentValueSubject<Data, Never>(res)
			group.addTask {
