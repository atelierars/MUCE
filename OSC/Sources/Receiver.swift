...
//	}
//}
public func TcpReceiver<Endpoint: IPEndpoint>(on endpoint: Endpoint, queue: Optional<DispatchQueue> = .none, count: Int = 1) -> some Publisher<(Packet, Endpoint), NWError> {
	TcpStream.Incoming(on: endpoint, count: count, queue: queue).flatMap {(stream, endpoint)in
		stream.compactMap { Packet(rawValue: $0).map { ($0, endpoint) } }
	}
}
